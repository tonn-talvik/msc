...
% ------------------------------------------------------------
  \begin{center}
    ...
    \vskip 3em 
    \LARGE\textsc{efektianalüüsidel põhinevate programmiteisenduste sertifitseerimine}\\
    \normalsize\vskip 4em 
    Magistritöö\\
    \vskip 4em    
  \end{center}
...
\todo{osaline järjestatus või eeljärjestatus?}
...
record GradedMonad : Set where
  ...
  T₁ f = sub-eq (sym ru) ∘ bind (η ∘ f)\end{verbatim}
...
\subsubsection{Keele rafineerimine}
...
\subsubsection{Liikmete tüübituletus}
...
... | nothing = ⊤
... | just τ = CTerm Γ τ\end{verbatim}
  \caption{Väärtus- ja arvutusliikmete efektituletuste tüübikonstruktorid.}
...
... | just _ | u = LAM σ u
    ... | nothing | u = tt\end{verbatim}
  \caption{Eranditega keele väärtusliikmete efektituletus.}
  \label{fig:exc.infer-vterm}
\end{figure}

\begin{figure}
  \begin{verbatim}
  \end{verbatim}
  \caption{Eranditega keele arvutusliikmete efektituletus.}
  \label{fig:exc.infer-cterm}
\end{figure}
...
⟦ VCAST t p ⟧v ρ = vcast p (⟦ t ⟧v ρ)\end{verbatim}
...
\item {\tt TRY_WITH_ {e} {e'} t t'} kombineerib osaarvutuste {\tt t} ja {\tt t'} interpretatsioonid vastavalt arvutuste efektidele. \todo{explain sor}
\item {\tt IF_THEN_ELSE_} korral interpreteeritakse tingimus, kusjuures kummagi haru efekt neeldub efektide ülemises rajas.
\item {\tt PREC x t t' p} interpretatsioon vastab primitiivsele rekursioonile, mille sammude arv on on väärtusliikme {\tt x} interpretatsioon, baas on arvutusliikme {\tt t} interpretatsioon ja sammuks on arvutusliikme {\tt t'} interpretatsioon kontekstis, kuhu on lisatud sammuloendur ja vahetulemuse akumulaator. \todo{explain primrecT}
...
⟦ CCAST t o ⟧c ρ = ccast o (⟦ t ⟧c ρ)\end{verbatim}
...
Monaadi spetsiifilised, efektist sõltumatud optimisatsioonid on toodud joonisel \ref{fig:exc.opt1}. {\tt the-same} näitab, et arvutust {\tt m} ei saa muuta, lisades sellele erandikäsitlejana sama arvutuse.
...
dead-comp m n ρ = lemmaC ρ (⟦ m ⟧c ρ) zero n\end{verbatim}
