...
Lõputöö on kirjutatud eesti keeles ning sisaldab teksti 46 leheküljel, 4 peatükki, \totalfigures~joonist.
\clearpage\vspace*{0pt}
...
The thesis is in Estonian and contains 46 pages of text, 4 chapters, \totalfigures~figures.
\clearpage\vspace*{0pt}
...
Arvutustermid {\tt CLC2-n-PAIR} ja {\tt CLC1-n-PAIR} (jn~\ref{fig:exc.raw.ex1}) eeldavad, et kontekstis on vähemalt üks funktsioon, mis võtab naturaalarvu ja tagastab naturaalarvu.
Seejuures pole oluline, kas see funktsioon õnnestub või mitte.
Arvutusterm {\tt CLC2-n-PAIR} rakendab kontekstis olevat funktsiooni 2 korda argumendile null.
Saadud tulemustest koostatakse paar.
Arvutusterm {\tt CLC1-n-PAIR} rakendab seda funktsiooni 1 korra argumendile null ning koostab paari, kus mõlemaks komponendiks on rakendamise tulemus.
Harjumuspärases süntaksis saab need termid esitada järgnevalt:
\begin{equation*}
\begin{split}
  \text{CLC2-n-PAIR} &:= \text{let}~x=f~0~\text{in}~\text{let}~y=f~0~\text{in}~\text{val}~(x,y)\\
  \text{CLC1-n-PAIR} &:= \text{let}~x=f~0~\text{in}~\text{val}~(x,x)
\end{split}
\end{equation*}
Nende arvutustermide samaväärsust näidatakse alapeatükis~\ref{ssec:exc.optimizations}.
...
Tüüpimised {\tt typing-cmplx} ja {\tt typing-smpl} väidavad, et termid {\tt CMPLX} ja {\tt SMPL} on hästi tüübitud ning tagastavad kindlasti tõeväärtuse.
Kumbki tüübituletus teostati kontekstis {\tt Γ₀}, mis koosneb ühest ainsast tõeväärtustüüpi muutujast.

Tüüpimised {\tt typing-clc2-n-pair} ja {\tt typing-clc1-n-pair} väidavad, et termid on hästi tüübitud ning tagastavad naturaalarvude paari.
Kumbki tüübituletus teostati kontekstis {\tt Γ₁}, mis koosneb ühest ainsast funktsioonist, mis võtab ja tagastab naturaalarvu ning mille õnnestumine pole staatiliselt teada.
...
Arvutustermid {\tt CLC2-n-PAIR} ja {\tt CLC1-n-PAIR} (jn~\ref{fig:exc.raw.ex1}) olid hästi tüübitud kontekstis {\tt Γ₁} (alaptk~\ref{sssec:exc.infer-type}, jn~\ref{fig:exc.typing}).
Seega saab neid terme selles kontekstis rafineerida.
Lihtsustus {\tt clc-twice} (jn~\ref{fig:exc.opt2}) väidab, et saadud rafineeritud termide interpretatsioonid on selles kontekstis samaväärsed.
Tõestus on lihtsustuse {\tt dup-comp} (jn~\ref{fig:exc.opt3}) rakendus kolmele argumendile:
\begin{itemize}
\item arvutusele, mis rakendab kontekstis olevat funktsiooni nullile, ja mida on selles samas kontekstis rafineeritud;
\item arvutusele, mis koostab kontekstis olevatest naturaalarvudest paari, ja mida rafineeritakse kontekstis, kuhu on lisatud kaks naturaalarvu võrreldes esialgse kontekstiga;
\item algse konteksti interpretatsioonile {\tt ρ}.
\end{itemize}
