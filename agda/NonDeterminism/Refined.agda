...
⊔-sym zero (suc n) = refl
⊔-sym (suc m) zero = refl
⊔-sym (suc m) (suc n) = cong suc (⊔-sym m n)
...
glb : (n n' : ℕ) {m : ℕ} → n ⊓ n' ≡ m → m ≤ n
glb zero n' refl = z≤n
glb (suc n) zero refl = z≤n
glb (suc n) (suc n') refl = s≤s (glb n n' refl)
...
mutual -- construct inequality proof from given lower and upper bound
  ...
  lbC (e / σ) (e' / σ') refl | _ | [ q ] | just x | [ q' ] = st-comp (glb e e' q) (lbV σ σ' q')
...
mutual -- value and computation terms
  ...
  data CTerm (Γ : Ctx) : CType → Set where
    ...
    CHOOSE : ∀ {e e' σ} → CTerm Γ (e / σ) → CTerm Γ (e' / σ) → CTerm Γ ((e + e') / σ)
