...
open import MyList
open import Data.Integer renaming (_≤?_ to _z≤?_; _≟_ to _z≟_; suc to zsuc; _+_ to _z+_; _*_ to _z*_)
...
β→ ((fst t) $ u) = β→ (fst t) $ β→ u
β→ ((snd t) $ u) = β→ (snd t) $ β→ u
β→ (lam σ e) = lam σ (β→ e)
β→ (p * r) = β→ p * β→ r
β→ (fst (var x)) = fst (β→ (var x))
β→ (fst (t $ u)) = fst (β→ (t $ u))
β→ (fst (p * r)) = β→ p
β→ (fst (fst t)) = fst (β→ (fst t))
β→ (fst (snd t)) = fst (β→ (snd t))
β→ (snd (var x)) = snd (β→ (var x))
β→ (snd (t $ u)) = snd (β→ (t $ u))
β→ (snd (p * r)) = β→ r
β→ (snd (fst t)) = snd (β→ (fst t))
β→ (snd (snd t)) = snd (β→ (snd t))
